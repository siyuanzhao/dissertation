...
def evaluate_bin_att(predictions, data, i_exp, I_subset=None,
    ...
    att = np.mean(yf[t==1]) - np.mean(yf[t==0])
    ...
    eff_pred = ycf_p - yf_p
    eff_pred[np.squeeze(t>0)] = -eff_pred[np.squeeze(t>0)]

    ate_pred = np.mean(eff_pred[t==1])
    atc_pred = np.mean(eff_pred[t==0])

    att_pred = np.mean(eff_pred[t==1])
    ...
    policy_value, policy_curve = \
        policy_val(t, yf, eff_pred, compute_policy_curve)
...
def evaluate(output_dir, data_path_train, data_path_test=None, binary=False, embeddings=False, rname=None):
    ...
    data_train = load_assistments_data(data_path_train, embeddings=embeddings, rname=rname)

    # Load test data
    if data_path_test is not None:
        if Log.VERBOSE:
            print 'Loading TEST data %s...' % data_path_test
        data_test = load_assistments_data(data_path_test, embeddings=embeddings, rname=rname)
    else:
        data_test = None
