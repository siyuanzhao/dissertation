...
class cfr_net(object):
    ...
    def _build_output(self, h_input, t, dim_in, dim_out, do_out, FLAGS):
        ...
        for i in range(0, FLAGS.n_out):
            ...
            if FLAGS.residual_block:
                if i == (FLAGS.n_out-1):
                    h_out.append(self.nonlin(z+h_input))
                else:
    ...
    def _build_output_graph(self, rep, t, dim_in, dim_out, do_out, FLAGS):
        ...
        if FLAGS.split_output:
            ...
            y0, weights_out0, weights_pred0 = self._build_output(rep0, t, dim_in, dim_out, do_out, FLAGS)
            y1, weights_out1, weights_pred1 = self._build_output(rep1, t, dim_in, dim_out, do_out, FLAGS)
            ...
            if FLAGS.residual_block:
                h_input = rep
                y, weights_out, weights_pred = self._build_output(h_input, t, dim_in, dim_out, do_out, FLAGS)
            else:
                h_input = tf.concat([rep, t],1)
                y, weights_out, weights_pred = self._build_output(h_input, t, dim_in+1, dim_out, do_out, FLAGS)
